...
**按名称浏览并修改任意单个道具** — 应用内置道具数据库，你可以直接搜索真正想要的东西，不用再去翻数字 ID。
...
填充也只会注入存档中标记为“已安装”的 DLC 内容。但内置道具数据库早于《丛林》，所以丛林专属道具
...
| **每次游戏更新** | 重新生成道具与鱼类数据库，批量填充才不会填错。 |
| **每个语言** | 拿游戏自己发布的本地化数据去核对每一个游戏内名词，而不是把我们的英文翻过去。 |

如果你想知道这笔钱具体能补上什么洞，最直接的例子是：**道具数据库停在「In the Jungle」（丛林）DLC
之前，因为我没有买那个 DLC**。丛林独有的道具既查不到名字，也不在批量填充的范围里。这是钱真的能
解决的问题。
