...
**货币** — 金币、贝、匠人之火、研究点数、Cooksta 粉丝数、信赖点数与伪造点数。可以按 ±10/100/1000 微调、
...
| 背包 | 通用物品 · 制作材料（鱼类部件 + DREDGE 研究部件／骨头） · 鲛人村 · 农场种子 · 已捕鱼类等级 |
...
支持。DLC 食材已经适配，而且应用只会注入你的存档中标记为“已安装”的那些 DLC 的内容。
...
- **批量撤销**，覆盖每一个批量操作。
- 写入任何内容之前都会先给出**更改预览**。
- 批量填充时**跳过易腐的畸变鱼**（DREDGE 联动的捕获物）——游戏在加载时会丢弃囤积的畸变鱼，硬填反而会把你
  真正钓到的鱼清掉。
