...
**재화** — 골드, 베이, 장인의 불꽃, 연구 포인트, Cooksta(쿡스타) 팔로워 수, 신뢰도 포인트, 가짜 포인트.
...
| | |
|---|---|
| 식당 | 보유 식재료 · 모든 식재료(DLC 반영) · 2호점 재고 · 직원 레벨 |
| 인벤토리 | 일반 아이템 · 제작 재료(물고기 부위 + DREDGE 연구 부품·뼈) · 어인족 마을 · 농장 씨앗 · 잡은 물고기 등급 |
...
네. DLC 식재료도 처리하며, 세이브에 설치된 것으로 기록된 DLC의 콘텐츠만 넣습니다.
...
- 일괄 채우기에서 **사라지는 돌연변이 물고기(DREDGE 컬래버 어종)는 건너뜁니다.** 게임이 세이브를 불러올 때
