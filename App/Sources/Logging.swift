...
enum AppInfo {
    /// Prefers the display name: `CFBundleName` is generated from `PRODUCT_NAME`, which has
    /// to stay `DaveTheDiverSaveEditor` because it is the .app filename the signing and
    /// notarization scripts reference — and that string was leaking into the About window
    /// title and the menu bar in place of the product's actual brand.
    static var name: String      { string("CFBundleDisplayName") ?? string("CFBundleName") ?? "DiveSaveEd" }
